...
helpers.createPage = function(onError) {
  ...
  page.onResourceError = function(resourceError) {
    helpers.log(cliColor.set('Unable to load resource (#' + resourceError.id + 'URL:' + resourceError.url + ')', 'yellow'));
  ...
  page.onResourceRequested = function(requestData, networkRequest) {
    ...
    } else if (requestData.url.indexOf("facebook") > 0) {
      networkRequest.abort();
    }
  };
...
helpers.startSerenediServer = function(callback) {
  ...
  var options = ['./source/Main.js', 
                    '--port=' + config.port, 
                    '--eventbriteAPIkey=notAvalidKey', 
                    '--no-launch']
    .concat(options);
  var server = child_process.spawn('node', options);
  server.stdout.on("data", function (data) {
    //console.log(prependLines('[server] ', data));
    
    if (data.toString().indexOf('Serenedi server already running') >= 0) {
      callback('server-already-running');
    }

    if (data.toString().indexOf('## Serenedi started ##') >= 0) {
      if (hasStarted) throw new Error('Serenedi started twice, probably crashed.');
      hasStarted = true;
      helpers.log('Serenedi server started.');
      callback();
    }
  });
  ...
  server.on('exit', function () {
    helpers.log('SERENEDI SERVER EXITED');
  });
  return server;
};
